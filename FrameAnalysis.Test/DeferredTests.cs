...
public class DeferredTests
{
    ...
    public void TestDeferred()
    {
        ...
        AreEqual(firstOwnedThing, first.TestProp, "First's thing can't be retrieved");

        AreEqual(firstOwnedThing, second.TestProp, "Second doesn't inherit property from first");

        AreEqual(first, firstOwnedThing.Owner, "First doesn't own its own thing");

        AreEqual(first, second.TestProp?.Owner, "First doesn't own second's thing");

        var secondOwnedThing = new OwnedThing();
        first.TestProp = secondOwnedThing;

        AreEqual(secondOwnedThing, first.TestProp, "First's thing hasn't been replaced");

        AreEqual(1, first.Deferred.Collisions.Count(), "Replacing the thing wasn't logged");

        second.AnotherProp = firstOwnedThing;

        AreEqual(firstOwnedThing, second.AnotherProp, "Second's thing can't be retrieved");

        AreEqual(second, firstOwnedThing.Owner, "Owner of thing wasn't updated to second");

        first.NotInterited = new MergableThing();
        first.NotInterited = new MergableThing();

        IsTrue(first.NotInterited.Merged, "First's merge prop not marked as merged");
    }
    ...
    public void TestDeferredMultiLayer()
    {
        ...
        AreEqual(firstOwnedThing, third.TestProp, "Third's thing can't be retrieved");

        AreEqual(third, first.TestProp.LastUser, "Third not registered as last user");

        first.NotInterited = new MergableThing();

        IsNull(third.NotInterited, "Third should not inherit First's AnotherProp value");
    }
}
