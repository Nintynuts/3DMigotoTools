...
{
    private readonly Regex includePattern = new("#include\\s+\"(?'path'.*?)\"");
    private readonly Regex constantBufferPattern = new(@"cbuffer\s+(?'name'\w+)\s*:\s*register\(b(?'slot'\d+)\)");
    private readonly Regex textureMacroPattern = new(@"TEXTURE(?:_CMP)?\(_(?'type'\w+),(?'name'\w+),\d+,(?'slot'\d+)\)");
    private readonly Regex texturePattern = new(@"(?'name'\w+)\s*:\s*register\(t(?'slot'\d+)\)");

    private readonly List<ShaderUsage<string>> includes = new();
    private readonly List<string> done = new();
    ...
    public void Scrape(DirectoryInfo shaderFixes)
    {
        ...
        {
            var hash = ulong.Parse(file.Name[..file.Name.IndexOf('-')], NumberStyles.HexNumber);
